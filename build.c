...
int main(int argc, char **argv)
{
	...
	if (chk_path("build/Release/config/") == 0 || chk_path("build/Release/logs/") == 0)
	{
			...
			exec_cmd("powershell -command mkdir build/Release/logs/");
		#else
	...
	{
		#ifdef _WIN32
			exec_cmd("powershell -command ni build/Release/logs/logfile.txt");
			exec_cmd("powershell -command ni build/Release/config/default_shell_type.txt");
			exec_cmd("powershell -command ni build/Release/config/shell_type.txt");
			exec_cmd("powershell -command ni build/Release/config/shell_usr.txt");
		#else
